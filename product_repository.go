...
func (r *ProductRepository) GetAllProducts(ctx context.Context) ([]models.Product, error) {
	rows, err := r.db.Query(ctx, "SELECT id, title, image_url, price, description, listen, quantity, is_favorite, in_cart FROM products")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		err := rows.Scan(&product.ID, &product.Title, &product.ImageURL, &product.Price, &product.Description, &product.Listen, &product.Quantity, &product.IsFavorite, &product.InCart)
		if err != nil {
			return nil, err
		}
...
func (r *ProductRepository) GetProductByID(ctx context.Context, id int) (*models.Product, error) {
	var product models.Product
	err := r.db.QueryRow(ctx, "SELECT id, title, image_url, price, description, listen, quantity, is_favorite, in_cart FROM products WHERE id = $1", id).
		Scan(&product.ID, &product.Title, &product.ImageURL, &product.Price, &product.Description, &product.Listen, &product.Quantity, &product.IsFavorite, &product.InCart)
	if err != nil {
		return nil, err
	}
	return &product, nil
...
func (r *ProductRepository) CreateProduct(ctx context.Context, product *models.Product) error {
	_, err := r.db.Exec(ctx, `
		INSERT INTO products (title, image_url, price, description, listen, quantity)
		VALUES ($1, $2, $3, $4, $5, $6, $7)
	`, product.Title, product.ImageURL, product.Price, product.Description, product.Listen, product.Quantity)
	return err
}

func (r *ProductRepository) UpdateProduct(ctx context.Context, product *models.Product) error {
	_, err := r.db.Exec(ctx, `
		UPDATE products SET title = $1, image_url = $2, price = $4, description = $5, listen = $6, quantity = $7
		WHERE id = $8
	`, product.Title, product.ImageURL, product.Price, product.Description, product.Listen, product.Quantity, product.ID)
	return err
}
...
func (r *ProductRepository) GetFilteredProducts(ctx context.Context, searchQuery string, minPrice, maxPrice float64, sortBy, sortOrder string) ([]models.Product, error) {
	query := `
		SELECT id, title, image_url, price, description, listen, quantity, is_favorite, in_cart
		FROM products
		WHERE ($1 = '' OR title ILIKE $1 OR title ILIKE $1)
		AND ($2 = 0 OR price >= $2)
		AND ($3 = 0 OR price <= $3)
	`
